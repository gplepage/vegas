...
Another reason to save results is to guard against the possibility of something 
going wrong with the integration after some number of reasonable iterations. 
Consider for example the following |vegas| output from an array-valued integrand with three 
elements:

.. literalinclude:: eg9c.out

Clearly something went wrong after the eigth iteration and the final results should
not be trusted. Assuming the 
integration results were stored in file ``'save.pkl'``, results from just 
the first eight iterations can be obtained using ::
...
.. literalinclude:: eg9d.out

The problem in the original analysis comes from the fact that the 
three elements of the integrand are exactly the same aside from an overall 
normalization::
...
The results are completely correlated, making their correlation matrix
singular. This can destabilize the weighted averages, expecially when the 
integrands differ dramatically in scale. |vegas| attempts to 
correct for this but here roundoff errors eventually cause a problem. The
result from averaging the first eight iterations is quite reasonable, however.
Of course, a better solution is not to include multiple copies of the 
same function in a multi-valued integrand.
