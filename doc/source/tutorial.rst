...
it is called. The compiled code is used in subsequent
...
Multiple Processors
---------------------------
|vegas| has built in support for MPI,
allowing the evaluation of integrands
to be spread over multiple processors. This can shorten execution time
substantially when the integrand is costly to evaluate.

MPI support in |vegas| relies upon Python module :mod:`mpi4py`,
which must be installed (together with MPI, of course).

To illustrate MPI parallel processing with |vegas|,
consider an integrand consisting of 1000 narrow Gaussians distributed evenly
along the diagonal of a 4-dimensional unit hypercube. We
implement the integrand in Cython, putting the result
in a file ``ridge_integrand.pyx``::

    # file: ridge_integrand.pyx
    ...
    def f(double[::1] x):
        ...
        cdef int N=1000
        cdef double ans = 0
        cdef double norm = (100. / 3.1415926535897932384626) ** 2 / N
        for j in range(N):
            x0 = j / (N - 1.)
            dx2 = 0.0
            for d in range(dim):
...
The main integration code,
in file ``ridge.py``, is then::

    # file: ridge.py

    import vegas

    # compile ridge_integrand.pyx, if needed, on import
    import pyximport
    ...
    def main():
        integ = vegas.Integrator(4 * [[0, 1]])
        # adapt
        integ(ridge_integrand.f, nitn=10, neval=1e5)
        # final results
        result = integ(ridge_integrand.f, nitn=10, neval=1e5)
        if integ.mpi_rank == 0:
            # result should be approximately 0.85
            print('result = %s    Q = %.2f' % (result, result.Q))

    if __name__ == '__main__':
        main()

The integrand and main program are completely standard,
except that we check the MPI
rank of the process to avoid printing out multiple copies of the result,
after the integration. To run this code on 4 CPUs, we execute ::

    mpirun -np 4 python ridge.py

which gives the output ::

    result = 0.85130(64)    Q = 0.50

This code runs about 3 times faster on 4 CPUs than on a single CPU.
...
It is important that the separate processes use the same random
numbers when running in parallel. This is automatically enforced
(unless :class:`vegas.Integrator` parameter ``sync_ran``
is set to ``False``) provided the default random-number generator
is used.
...
in function to :class:`vegas.BatchIntegrand`, but where Python's
:mod:`multiprocessing` module provides parallel processing, is::
...
implementation of parallelism is not as efficient as the built-in
MPI implementation.
...
---------------------------
Integrands coded in C or Fortran are often very fast to evaluate. There are
many ways to access such integrands using the Python |vegas|. Here we
