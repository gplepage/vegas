...
-------------------------------------
|vegas| integrators generate random points in the integration volume from a 
distribution that is optimized for integrals of whatever function it 
was trained on. It provides low-level access to the random-point generator
...
Here ``x[i, d]`` is an array of integration points, and ``wgt[i]`` the 
